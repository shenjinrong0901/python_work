                ...
                int max_list_size)
{
    ...
    if (!qual_map) {   /* don't order if qual_map is NULL */
        ...
        else
        {
            ...
            else if (quality > qual_map[index_list[n - 1]])
            {
                ...
                {
                    mid = (top + bot)/2;
                    if (quality <= qual_map[index_list[mid]])
                        bot = mid;
                    else
                        top = mid;
                }
    ...
    if (qual_map && min_qual && max_list_size > 0 && *num_index >= max_list_size)
    {
...
/* pixel is "val".                                       */
void UpdateList(float *qual_map,
                int x,
                int y,
                float val,
                float *phase,
                ...
                float *min_qual)
{
    ...
    if (a >= 0 && !(bitflags[k] & (ignore_code | processed_code)))
    {
        w = y*xsize + x-1;
    ...
    if (a < xsize && !(bitflags[k] & (ignore_code | processed_code)))
    {
        w = y*xsize + x;
        grad = - Gradient(phase[w], phase[w+1]);
        //if (dxdy_flag && qual_map)
    ...
    if (b >= 0 && !(bitflags[k] & (ignore_code | processed_code)))
    {
        w = (y-1)*xsize + x;
    ...
    b = y + 1;
    k = b*xsize + a;
    if (b < ysize && !(bitflags[k] & (ignore_code | processed_code)))
    {
        w = y*xsize + x;
        grad = - Gradient(phase[w], phase[w+xsize]);
        //if (dxdy_flag && qual_map)
